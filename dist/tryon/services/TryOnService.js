 ...
 */
async function preprocessGarmentLengthLocked(garmentUrl) {
...
class TryOnService {
    async generateTryOn(input) {
        ...
        // --------------------------------------------------
        const finalGarmentUrl = await preprocessGarmentLengthLocked(input.garmentImageUrl);
        // --------------------------------------------------
        // 1️⃣ SUBMIT JOB TO FASHN
        // --------------------------------------------------
        const runResponse = await axios_1.default.post(FASHN_RUN_URL, {
