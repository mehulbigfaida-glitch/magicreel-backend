...
export interface GarmentHygieneResult {
  cleanGarmentUrl: string;
  mannequinDetected: boolean;
}

export class GarmentHygieneService {
  /**
  ...
  async cleanGarment(
    ...
    const mannequinMask = await sharp(grayscale)
    ...
    const stats = await sharp(mannequinMask).stats();
    const darkPixelRatio =
      stats.channels[0].mean < 240 ? true : false;
