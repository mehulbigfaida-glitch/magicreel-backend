...
interface MagicReelConcatInput { clips: string[]; outputDir: string; }
...
export class MagicReelConcatService {
  async generateMagicReel(input: MagicReelConcatInput): Promise<string> {
    ...
    for (let i = 1; i < clips.length; i++) {
      const offset = i * (clipDuration - fadeDuration);
      const out = i === clips.length - 1 ? "[v]" : `[v${i}]`;
      filterParts.push(`${current}[${i}:v]xfade=transition=fade:duration=${fadeDuration}:offset=${offset}${out}`);
      current = out;
    }

    const ffmpegCommand = `"${ffmpegPath}" -y ${inputs} -filter_complex "${filterParts.join(";")}" -map "[v]" -c:v libx264 -preset medium -pix_fmt yuv420p -movflags +faststart "${outputVideoPath}"`;
    console.log("🎞️ Crossfade Reel FFmpeg");
    console.log(`🎞️ CONCAT CLIPS: ${clips.length}`);
    console.log(`🎞️ EXPECTED DURATION: ${clips.length * clipDuration - (clips.length - 1) * fadeDuration}s`);
